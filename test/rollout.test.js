...
test('waitForDeploymentRollout completes after readiness on an arbitrary path', async () => {
  let calls = 0;
  const client = { request: async () => {
    calls += 1;
    return calls === 1
      ? { metadata: { generation: 2 }, spec: { replicas: 1 }, status: { observedGeneration: 2, updatedReplicas: 1 } }
      : { metadata: { generation: 2 }, spec: { replicas: 1 }, status: { observedGeneration: 2, replicas: 1, updatedReplicas: 1, readyReplicas: 1, availableReplicas: 1 } };
  }};
  ...
  assert.equal(result.deployment.spec.replicas, 1);
});
